...
### 폐기된 OCR 복구 제안 (2026-08-25)

이 제안은 최신 이슈 댓글의 MVP 확정 사항과 충돌하므로 폐기한다. 아래 OCR 관련
결정·구현 단계·영향 범위·테스트 계획은 과거 이력이며, 현재 구현 기준이 아니다.
...
7. [x] **Phase 7 (완료): 최종 브랜치 OCR 회귀 복구**
   - P6 이후 OCR을 제거한 커밋으로 인해 최종 브랜치에서 사라진 이미지 업로드,
     텍스트 기반 조항 검출, multipart 웹 연결을 복구한다.
   - PNG/JPEG 수신, 비이미지·10MB 초과 거부, 추출 실패 전파, 0건 검출 위험도,
     웹 재시도와 유형별 체크 상태 저장을 테스트로 검증한다.
   - 모든 분석 결과와 화면에 “참고용이며 법적 효력 없음” 고지를 유지한다.
   - 커밋: `feat: [P7] #69 최종 브랜치 OCR 계약서 분석 복구`
