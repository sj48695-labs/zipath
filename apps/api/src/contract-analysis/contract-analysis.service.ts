...
export class ContractAnalysisService {
  ...
  analyzeText(type: string, text: string): ContractAnalysisResult {
    ...
    const missingRequired = clauses
      .filter((clause) => !clause.detected && clause.severity === "required")
      .map((clause) => clause.label);
    const missingRecommended = clauses
      .filter((clause) => !clause.detected && clause.severity === "recommended")
      .map((clause) => clause.label);
    const riskLevel =
    ...
    return {
      ...
      riskSummary:
        riskLevel === "danger"
          ? `필수 조항 ${missingRequired.length}개가 확인되지 않았습니다. 누락 여부를 점검하세요.`
          : riskLevel === "caution"
            ? "필수 조항은 확인됐지만 권장 조항 일부가 누락되었습니다."
            : "주요 조항이 모두 확인되었습니다.",
